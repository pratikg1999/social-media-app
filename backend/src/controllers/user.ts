...
const getUsers = async (request: Request, response: Response, next: NextFunction) => {
    try {
        let followings = (await UserModel.findById(request.userId))?.followings;
        // let users = await UserModel.find().map((doc) => ({ ...doc, isFollowing: "hlee" }));
        ...
        // console.log(users._id, users.isFollowing);

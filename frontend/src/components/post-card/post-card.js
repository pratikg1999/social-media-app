...
const PostCard = (props) => {
  ...
  const isLiked =
    ...
    props.post.likes.map((e) => e._id).includes(props.currentUserInfo._id);
        ...
        <CardHeader
          avatar={
            <Avatar
              className="bg-danger"
              alt={
                props.post.createdBy.firstName[0].toUpperCase() +
                props.post.createdBy.lastName[0].toUpperCase()
              }
              src={axios.defaults.baseURL + props.post.createdBy.profileImage}
            />
          }
          title={
            <span
              onClick={() => {
                props.history.push(`/profile/${props.post.createdBy._id}`);
              }}
              style={{ cursor: "pointer" }}
            >
              {props.post.createdBy.firstName +
                " " +
                props.post.createdBy.lastName}
            </span>
          }
          subheader={
            props.post.creationTime
              ? new Date(props.post.creationTime).toDateString()
              : ""
          }
          action={
            props.currentUserInfo !== undefined &&
            props.currentUserInfo._id === props.post.createdBy._id ? (
              <>
                <IconButton
                  aria-label="settings"
                ...
                >
                  ...
                  <MenuItem
                    ...
                    data-toggle="modal"
                    data-target={`#modal${props.post._id}`}
                  >
                    Edit
                  </MenuItem>
                </Menu>
          ...
          <div className="px-3" style={{ fontSize: "small", color: "grey" }}>
            <div className="d-table-row">
              ...
              {props.post.likes.length - 3 > 0 && (
                <span
                  className="d-table-cell pl-1"
                  style={{ verticalAlign: "middle" }}
                >
                  and {props.post.likes.length - 3} others
                </span>
              )}
            </div>
          </div>
        ...
        <CardActions className="pt-0">
          <div>
            <IconButton
              ...
              onClick={() => props.changePostLike(props.post._id, !isLiked)}
            >
              <Favorite />
            </IconButton>
          ...
          <CardContent>
            <Grid item xs={12}>
              {props.comments.map((comment) => {
                ...
                return props.currentUserInfo !== undefined &&
                  props.currentUserInfo._id === comment.createdBy._id ? (
                  <Comment
                    {...comment}
                  ...
                  <Comment
                    ...
                    key={
                      comment.firstName +
                      comment.createdBy.lastName +
                      comment.createdBy.creationTime
                    }
                  />
                );
              })}
        ...
        <div className="modal-dialog modal-dialog-centered" role="document">
          <div className="modal-content">
            <div className="modal-header">
              <h5 className="modal-title" id={`modal${props.post._id}_title`}>
                Edit Post
              </h5>
              <button
                type="button"
            ...
            <div className="modal-body">
              {" "}
              <EditPost onEdit={props.onEdit} post={props.post} />{" "}
            </div>

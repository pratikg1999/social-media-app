...
class EditPost extends React.Component {
  ...
  componentDidMount() {
    if (this.props.post.image) {
      this.setState({
        profileImage: axios.defaults.baseURL + this.props.post.image,
      });
    }
  }
  ...
  submitForm = async (e) => {
    ...
    if (e.target.body.value || e.target.image.value) {
      ...
      console.log(
        ...
        e.target.image.value === undefined
      );
      if (!e.target.image.value) {
        if (this.state.profileImage) {
          formData.set(
            ...
            new Blob([
              (
                await axios.get(this.state.profileImage, {
                  responseType: "blob",
                })
              ).data,
            ]),
          ...
          <Grid container className="p-3">
            ...
            <Grid item xs={12}>
              <input
                ...
                accept="image/*"
                className="d-none"
                id={`inputimg_${this.props.post._id}`}
                type="file"
